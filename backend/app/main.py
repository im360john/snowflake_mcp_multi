...
async def mcp_messages(
    ...
    if not message_queue and method in ["ping", "initialize"]:
        logger.warning(f"No SSE connection for {connection_id}, but received {method} - responding directly")
        
        # For ping, just return 200 OK with no body
        if method == "ping":
            return Response(status_code=200)
        
        # For initialize, return a basic response
